...
__global__ void dot_array_p1(const int *dinput, int *doutput) {
    volatile __shared__ int shared_output[THREAD_PRE_BLOCK]; 
    ...
    //最後剩下32thread，就不會有跨warp之間資料錯誤的問題，所以可以拆開來執行，省去四次同步
    if(threadIdx.x < 32) {
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 32];
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 16];
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 8];
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 4];
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 2];
        shared_output[threadIdx.x] += shared_output[threadIdx.x + 1];
    }
